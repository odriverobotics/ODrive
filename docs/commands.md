...
* View encoder position with `<axis>.encoder.pos_estimate` [counts] or `<axis>.encoder.pos_est_rad` [radians]
* View rotational velocity with `<axis>.encoder.vel_estimate` [counts/s] or `<axis>.encoder.vel_est_rad` [radians]
