...
If you are using a motor to provide a large braking torque at low RPM then you may also encounter this error. This will occur if the current required to be absorbed by the brake resistor exceeds the voltage generated by the motor divided by the brake resistor resistance. e.g. A 150 kV motor rotating at 200 RPM will produce 1.33 V and so the maximum current that can be absorbed by a 2 Ohm brake resistor is 0.66 A (< 1W).
