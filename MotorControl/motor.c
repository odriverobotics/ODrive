...
// TODO: Migrate to C++, clearly we are actually doing object oriented code
// here...
...
Motor_t motors[] = {
    {
        ...
        .motor_timer = &htim1,
        .next_timings = {TIM_1_8_PERIOD_CLOCKS / 2, TIM_1_8_PERIOD_CLOCKS / 2,
        ...
        .sensorless =
            {
                ...
                .pm_flux_linkage = 1.58e-3f,          // [V / (rad/s)]  { 5.51328895422
                                                      // / (<pole pairs> * <rpm/v>) }
...
bool calibrate_motor(Motor_t *motor) {
    ...
    if (!measure_phase_resistance(motor, motor->calibration_current, 1.0f))
        return false;
    if (!measure_phase_inductance(motor, -1.0f, 1.0f)) return false;
    if (motor->rotor_mode == ROTOR_MODE_ENCODER ||
        motor->rotor_mode == ROTOR_MODE_RUN_ENCODER_TEST_SENSORLESS) {
        if (!calib_enc_offset(motor,
                              motor->calibration_current * motor->phase_resistance))
            return false;
    }
...
void calculate_current_gains(Motor_t *motor) {
    ...
    motor->current_control.p_gain =
        current_control_bandwidth * motor->phase_inductance;
...
bool calculate_pll_gains(Motor_t *motor) {
    ...
    // Critically damped
    motor->encoder.pll_ki =
        0.25f * (motor->encoder.pll_kp * motor->encoder.pll_kp);
...
bool measure_phase_resistance(Motor_t *motor, float test_current,
                              float max_voltage) {
...
// Melon Refactor - Good
bool measure_phase_inductance(Motor_t *motor, float voltage_low,
                              float voltage_high) {
...
bool calib_enc_offset(Motor_t *motor, float voltage_magnitude) {
    ...
    } else if ((int16_t)motor->encoder.encoder_timer->Instance->CNT <
               init_enc_val - 8) {
...
bool check_deadlines(Motor_t *motor) {
    // Check we meet deadlines after queueing
    motor->last_cpu_time = check_timing(motor);
    if (!(motor->last_cpu_time < motor->control_deadline)) return false;
    return true;
}