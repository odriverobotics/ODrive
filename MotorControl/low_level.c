...
void update_sensorless(Motor_t *motor)
{
    ...
    sensorless->pll_pos = wrap_pm_pi(sensorless->pll_pos + get_current_meas_period() * sensorless->pll_vel);
    ...
    sensorless->pll_pos = wrap_pm_pi(sensorless->pll_pos + get_current_meas_period() * sensorless->pll_kp * delta_phase);
    // update PLL velocity
    sensorless->pll_vel += get_current_meas_period() * sensorless->pll_ki * delta_phase;
...
static bool spin_up_sensorless(Motor_t *motor)
{
    ...
    {
        vel += motor->sensorless.spin_up_acceleration * get_current_meas_period();
...
void control_motor_loop(Motor_t *motor)
{
    while (motor->enable_control)
    {
        ...
        else
        {
            ...
            {
                motor->vel_integrator_current += (motor->vel_integrator_gain * get_current_meas_period()) * v_err;
            }
        }
...
static bool FOC_current(Motor_t *motor, float Id_des, float Iq_des)
{
    ...
    {
        ictrl->v_current_control_integral_d += Ierr_d * (ictrl->i_gain * get_current_meas_period());
        ictrl->v_current_control_integral_q += Ierr_q * (ictrl->i_gain * get_current_meas_period());
    }
...
void motor_thread(void const *argument)
{
    ...
    for (;;)
    {
        ...
        if (motor->calibration_ok && motor->enable_control)
        {
            ...
            if (motor->enable_control)
            { // if control is still enabled, we exited because of error
