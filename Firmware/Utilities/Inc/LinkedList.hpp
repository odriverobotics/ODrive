...
public:
    ...
    int32_t updateNode(uint32_t nodeID, MyType * pData);
...
inline CLinkedList<MyType>::CLinkedList(void * pTable, size_t size)
    ...
    , m_pHead(nullptr)
{
    memset((void *)m_pTable, 0, size);
    for(auto i = 0u; i < m_length; ++i)
    {
        m_pTable[i].setNext((i < (m_length - 1u)) ? &m_pTable[i + 1] : nullptr);
...
/**\brief   Updates the data in an existing specific node
 *
 * \param   nodeID  - node to update, 0 is head
 * \param   pData   - pointer to the data to copy in
 *
 * \return  LL_SUCCESS on successful allocation else LL_FAIL
 */
template <class MyType>
inline int32_t CLinkedList<MyType>::updateNode(uint32_t nodeID, MyType * pData)
{
    int32_t returnVal = LL_FAIL;
