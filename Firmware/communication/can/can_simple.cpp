    ...
    for (auto& handler: axis.config_.can.periodic_handlers)
    {
        if (handler.can_command == periodic_cmd)
        {
            ...
            else
            {
                ...
                canbus_->release_sem(); // Release semaphrore to reload periodic task timer
            }
            break;
        }
...
uint32_t CANSimple::service_stack() {
    ...
    for (auto& a: axes) {
        MEASURE_TIME(a.task_times_.can_heartbeat) {
            for (auto& handler : a.config_.can.periodic_handlers)
            {
                if (!handler.enabled)
                {
                    break; // Reached end of activated handlers
                }

                if ((now - handler.last_call) >= handler.call_rate_ms)
                {
                    if (call_periodic_handler(a, handler.can_command) >= 0)
                    {
                        handler.last_call = now;
                    }
                    int nextAxisService = handler.last_call + handler.call_rate_ms - now; // Add protection for roll over?
