...
bool SensorlessEstimator::update() {
    ...
    for (auto i = 0; i <= 1; ++i) {
        ...
        auto y = -axis_->motor_.config_.phase_resistance * I_alpha_beta[i] + V_alpha_beta_memory_[i];
    ...
    auto pll_kp = 2.0f * config_.pll_bandwidth;
    // Critically damped
    auto pll_ki = 0.25f * (pll_kp * pll_kp);
